...
docker run --rm --mount type=bind,source=/Volumes/crb2,target=/data --mount type=bind,source=${PWD},target=/app --mount type=bind,source=${HOME}/.aws,target=/root/.aws -it artifactory.huit.harvard.edu/lts/crb_validator:latest $@
